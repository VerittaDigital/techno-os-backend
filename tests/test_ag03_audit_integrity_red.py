...
class TestAg03AuditIntegrityRed:
    ...
    def test_capability_mismatch_preserves_auditability(
        ...
        # Avoid file IO side-effects during tests and capture logger calls directly
        monkeypatch.setattr("app.audit_sink.append_audit_record", lambda record: None)
        mock_info = MagicMock()
        monkeypatch.setattr("app.action_audit_log.logger.info", mock_info)
        ...
        # action_audit MUST be emitted (via logger.info)
        assert mock_info.called, "action_audit logger.info must be called"
        audit_json = json.loads(mock_info.call_args[0][0])
    ...
    def test_trace_id_correlation_across_audits(
        ...
        # Extract audit logs via mocked logger.info
        assert mock_info.called, "action_audit logger.info must be called"
        action_audit = json.loads(mock_info.call_args[0][0])
...
class TestAg03AuditIntegrityRed:
    ...
    def test_capability_mismatch_preserves_auditability(
        ...
        - action_audit emitted (BEFORE any HTTP exception)
        - action_audit.status == "BLOCKED"
            # Capture logger.info calls directly to avoid caplog ordering flakiness
            mock_info = MagicMock()
            monkeypatch.setattr("app.action_audit_log.logger.info", mock_info)
