# F8.6.1 — OpenTelemetry Safe Implementation Plan

## 🚨 CONTEXTO: Lições do Incidente Catastrófico (2025-12-30)

**O que deu errado**:
- Script automatizado modificou código sem testes
- Injetou `_end_span()` sem definir a função
- Zero validação antes de commit
- 174 arquivos contaminados (formatting noise)
- NameError em runtime → backend quebrado

**Princípios para evitar repetição**:
1. ✅ **Manual-only instrumentation** (ZERO automation)
2. ✅ **Test-first approach** (testes antes de instrumentar)
3. ✅ **Incremental rollout** (1 módulo por vez)
4. ✅ **Code review humano** (linha-a-linha)
5. ✅ **Fail-closed governance** (backup before changes)

---

## 📋 ESTRATÉGIA: Incremental Instrumentation (7 Fases)

### Meta Global
Adicionar distributed tracing com OpenTelemetry **sem quebrar funcionalidade existente**.

### Completion Target
+1% (87.5% → 88.5%) — F8.6.1 implementado e validado

---

## 🔐 REGRAS ABSOLUTAS (NÃO NEGOCIÁVEIS)

### 1. Fail-closed obrigatório
- `TRACING_ENABLED=0` por padrão (env var)
- Se tracing falhar, sistema continua funcional
- Nenhuma dependência lógica em spans

### 2. Manual-only
- **PROIBIDO** qualquer script automatizado de modificação de código
- Instrumentação linha-a-linha, com code review humano

### 3. Incremental estrito
- 1 módulo por fase
- Nenhuma fase avança sem `pytest 100% PASS`
- Rollback testado ao menos 1x por fase

### 4. Wrapper-only
- Nenhum span direto em funções existentes
- Spans somente via wrapper `observed_span()` (não `create_span` direto)

### 5. Backup antes de cada fase
- `git tag PRE-PHASE-N` obrigatório
- Rollback deve ser executado e validado

### 6. Semântica imutável
- Outputs funcionais idênticos com tracing ON ou OFF
- Comparação explícita de outputs em testes

### 7. Human code review linha-a-linha
- Zero confiança em automação
...
❌ Scripts de rewrite  
❌ Instrumentação em massa  
❌ Alteração de fluxo decisório  
❌ Span criando dependência lógica  
❌ "Só um ajuste rápido"  

---

## 🔐 FASE -1 — GOVERNANÇA (OBRIGATÓRIA, 30min)

### Objetivo
Validar que sistema está saudável ANTES de iniciar instrumentação.

### Checklist Pré-Requisito
...
# Expected: 341 tests, ≥99% passing
```

#### 4. Env vars baseline
```bash
# Verificar que tracing está desabilitado
...
```bash
# Criar arquivo temporário e rollback
...
- ✅ Baseline: 341 tests, ≥340 passing
- ✅ `TRACING_ENABLED=0` e `OBSERVED_PIPELINE_ENABLED=0`
- ✅ Rollback testado e funcional
- ✅ Backup completo criado

### ⚠️ CRITÉRIO DE BLOQUEIO
Se **qualquer item falhar** → **ABORTAR implementação** até resolver.

---

## 🎯 FASE 0 — PREPARAÇÃO (Pré-requisito, 2-3h)

### Objetivo
Configurar infraestrutura OpenTelemetry **sem tocar código de produção**.
...
#### 3. Criar módulo tracing (SEM instrumentar ainda)
```python
# app/tracing.py
"""
...
- ✅ `TRACING_ENABLED=0` confirmado (tracing desabilitado por padrão)
- ✅ Rollback testado e validado

### Rollback Execution (OBRIGATÓRIO)
```bash
...
- ✅ Jaeger container UP (`docker ps | grep jaeger`)
- ✅ Jaeger UI acessível (http://localhost:16686)
- ✅ Módulo `app/tracing.py` criado (SEM uso em produção ainda)
- ✅ Testes `test_tracing.py` 100% PASS
- ✅ Import test OK: `python3 -c "from app.tracing import init_tracing; print('OK')"`
- ✅ Backend continua funcionando (zero mudanças em código de produção)
...
## 🎯 FASE 1 — INSTRUMENTAÇÃO MAIN (Entry Point, 3-4h)
...
### Success Criteria (FASE 1)
- ✅ Testes `test_tracing_integration.py` 100% PASS
- ✅ Backend inicia sem erros (`uvicorn app.main:app`)
- ✅ Endpoint `/process` funciona normalmente
- ✅ Jaeger UI mostra spans (http://localhost:16686 → Search → Service: techno-os-backend)
- ✅ Suite completa backend: **341/341 PASS** (sem regressões)
...
## 🎯 FASE 2 — INSTRUMENTAÇÃO PIPELINE (Core Logic, 4-5h)

### Objetivo
Adicionar spans no **agentic_pipeline.py** (função principal).
...
- ✅ Testes `test_agentic_pipeline.py` continuam 100% PASS (12/12)
- ✅ Testes `test_agentic_pipeline_tracing.py` 100% PASS
- ✅ Suite completa backend: **342/342 PASS** (1 novo teste, zero regressões)
- ✅ Jaeger UI mostra hierarquia de spans (7 spans por execução)
- ✅ Backend funciona normalmente (sem tracing = no-op contexts)
...
# Jaeger UI: Verificar trace com 7+ spans hierárquicos
```
...
## 🎯 FASE 3 — INSTRUMENTAÇÃO EXECUTORS (High-Value, 3-4h)

### Objetivo
Adicionar spans nos **executors** (onde a execução real acontece).
...
#### 2. Repetir para outros executors críticos
- `composite_executor_v1.py`
- `test_executor_v1.py`
- Adicionar 1 executor por vez, validar, commit

### Success Criteria (FASE 3)
- ✅ Executor tests continuam passando (sem regressões)
- ✅ Jaeger UI mostra spans detalhados de executors
- ✅ Latency breakdown visível (API call vs validation vs etc)

---

## 🎯 FASE 4 — INSTRUMENTAÇÃO GATE (Governance, 2-3h)

### Objetivo
Adicionar spans no **gate_engine.py** (decisão ALLOW/DENY).
...
## 🎯 FASE 5 — CONTEXT PROPAGATION (Advanced, 2-3h)
...
```python
# app/agentic_pipeline.py (modificar execução)
...
## 🎯 FASE 6 — PRODUCTION HARDENING (Final, 2-3h)

### Objetivo
Configurar tracing para produção (sampling, performance tuning).
...
# Sample 10% of traces (configurável via env var)
...
#### 2. Configurar exporter para produção
```python
# Produção: Use OTLP exporter (mais eficiente que Jaeger Thrift)
...
## 📊 VALIDAÇÃO FINAL (Acceptance Criteria)

### Functional Requirements
- ✅ Backend funciona normalmente (com ou sem tracing)
- ✅ Jaeger UI mostra traces end-to-end
- ✅ Latency breakdown visível (7+ spans por request)
- ✅ Context propagation entre threads funciona
- ✅ Sampling configurável (default 10%)

### Non-Functional Requirements
- ✅ Zero regressões (suite completa 100% PASS)
- ✅ Overhead <5% (comparar P95 latency antes/depois)
- ✅ Fail-closed: Backend continua se tracing falhar
- ✅ Privacy: Zero PII em span attributes

### Governance Requirements
- ✅ Cada fase commitada separadamente
- ✅ Cada commit passa testes completos
- ✅ Code review humano linha-a-linha
- ✅ Rollback plan testado em cada fase
...
### Checkpoint Obrigatório (Cada Fase)
```bash
...
# Se qualquer check falhar → STOP, rollback, debug
```

### Backup Before Each Phase
```bash
PHASE="F8.6.1-PHASE-N"
BKP_DIR="/mnt/d/Projects/backups-techno-os/tracing-${PHASE}-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BKP_DIR"

# Backup files que serão modificados
cp app/main.py "$BKP_DIR/"
cp app/agentic_pipeline.py "$BKP_DIR/"
# ... outros arquivos

# Git tag antes de mudanças
git tag "PRE-${PHASE}"
```
...
| Fase | Esforço | Risco | Completion |
|------|---------|-------|------------|
| FASE 0 — Preparação | 2-3h | LOW | 87.5% → 87.7% |
| FASE 1 — Main | 3-4h | MEDIUM | 87.7% → 88.0% |
| FASE 2 — Pipeline | 4-5h | HIGH | 88.0% → 88.3% |
| FASE 3 — Executors | 3-4h | MEDIUM | 88.3% → 88.5% |
| FASE 4 — Gate | 2-3h | LOW | 88.5% → 88.6% |
| FASE 5 — Context | 2-3h | MEDIUM | 88.6% → 88.7% |
| FASE 6 — Hardening | 2-3h | LOW | 88.7% → 88.5% (final) |
| **TOTAL** | **18-25h** | — | **+1.0%** |

**Distribuição recomendada**: 3-4 dias (6h/dia), com pausas entre fases para validação.

---

## 🚀 PRÓXIMO PASSO IMEDIATO

**Recomendação**: Iniciar **FASE 0 (Preparação)** imediatamente.
...
# 4. Criar módulo tracing (sem instrumentar produção ainda)
# Copiar código de app/tracing.py desta documentação

# 5. Criar testes
# Copiar código de tests/test_tracing.py desta documentação

# 6. Validar
pytest tests/test_tracing.py -v
python3 -c "from app.tracing import init_tracing; print('✅ FASE 0 OK')"
```

**Após FASE 0 completa**: Decision point para continuar FASE 1 ou revisar approach.
...
### RUNBOOKs a Criar
- `RUNBOOK-TRACING-F8.6.1.md` — Comandos operacionais Jaeger
- `TRACING-TROUBLESHOOTING.md` — Debug de spans, latency analysis

### ADRs a Criar
- `ADR-F8.6.1-001-MANUAL-INSTRUMENTATION.md` — Por quê manual only
- `ADR-F8.6.1-002-FAIL-CLOSED-DESIGN.md` — Tracing como optional feature
- `ADR-F8.6.1-003-SAMPLING-STRATEGY.md` — 10% sampling rate justification
...
1. ✅ Zero automation (manual only)
2. ✅ Incremental rollout (6 fases isoladas)
3. ✅ Test-first (validação em cada fase)
4. ✅ Fail-closed (backend continua se tracing falhar)
5. ✅ Rollback ready (backups + git tags)
6. ✅ Code review humano (linha-a-linha)

**Diferença vs incidente F8.6.1**:
- ❌ ANTES: Script automatizado → 174 arquivos quebrados → NameError
- ✅ AGORA: Manual, incremental, testado → Zero risco de quebra catastrófica

**Confiança**: 95% (baseado em governança fail-closed + lições aprendidas)
